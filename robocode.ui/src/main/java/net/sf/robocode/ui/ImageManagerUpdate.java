...
public class ImageManagerUpdate implements IImageManager {
	...
	public RenderImage getExplosionRenderImage(int which, int frame) {
		if (explosionRenderImages == null) {
			...
			java.util.List<java.util.List<RenderImage>> explosions = new ArrayList<java.util.List<RenderImage>>();

			boolean done = false;

			for (numExplosion = 1; !done; numExplosion++) {
				java.util.List<RenderImage> frames = new ArrayList<RenderImage>();
